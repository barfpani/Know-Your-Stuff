...
CREATE TABLE recommendation_logs(
    id SERIAL PRIMARY KEY,
    product_id INTEGER NOT NULL REFERENCES products(id) ON DELETE CASCADE,
    current_listing_id INTEGER REFERENCES product_listings(id) ON DELETE SET NULL,
...
CREATE INDEX idx_product_listings_retailer_id ON product_listings(retailer_id);
CREATE INDEX idx_price_history_listing_id ON price_history(product_listing_id);
CREATE INDEX idx_price_history_captured_at ON price_history(captured_at);
CREATE INDEX idx_sales_events_date ON sales_events(start_date, end_date);